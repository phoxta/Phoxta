...
import { lookupReservation, type ReservationLookup } from '@/integration/phoxta'

const STATUS_LABEL: Record<string, string> = {
  pending: 'Pending', confirmed: 'Confirmed', completed: 'Completed', cancelled: 'Cancelled',
}

export default function PageManageBooking({ searchParams }: RouteProps) {
  ...
  // Mount-only: a later query change means a fresh navigation to this page.
  useEffect(() => {
      ...
      <div className="container mx-auto max-w-2xl">
        ...
        <form className="mt-10 grid grid-cols-1 gap-6 rounded-2xl border border-border p-6 sm:p-8" onSubmit={lookup}>
          <Field className="block">
            ...
            <Input type="text" className="mt-1" value={ref} onChange={(e: ChangeEvent<HTMLInputElement>) => setRef(e.target.value)} />
          </Field>
          <Field className="block">
            <Label>Email</Label>
            <Input type="email" className="mt-1" value={email} onChange={(e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} />
          </Field>
