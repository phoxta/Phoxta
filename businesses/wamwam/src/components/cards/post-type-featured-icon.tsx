...
function MediaIcon({ postType, iconSize }: { postType: string; iconSize: string }) {
    if (postType === "video") {
        return (
...
export default function PostTypeFeaturedIcon({
    className = "",
    postType = "standard",
    onClick,
    wrapSize = "w-11 h-11",
    iconSize = "w-6 h-6",
}: PostTypeFeaturedIconProps) {
    return (
        <div className={`nc-PostTypeFeaturedIcon ${className}`} data-nc-id="PostTypeFeaturedIcon" onClick={onClick}>
            {/* Standard posts render the wrapper only, so the layout slot stays in place. */}
            {!!postType && postType !== "standard" && (
                <span
                    className={`flex items-center justify-center rounded-full border border-white bg-neutral-900/60 text-xl text-white ${wrapSize}`}
                >
                    <MediaIcon postType={postType} iconSize={iconSize} />
                </span>
            )}
        </div>
    );
