...
import type { TExcludeDate } from "@/types/domain";

interface DateInputPopoverProps {
    className?: string;
}

export default function DateInputPopover({ className = "flex-1" }: DateInputPopoverProps) {
    const [startDate, setStartDate] = useState<Date | null>(new Date());
    // Upstream's demo exclusions. The second was the string `new Date('1-06-01')`,
    // which only V8 parses (to 6 Jan 2001); spelt out so every engine agrees.
    const excludeDates: TExcludeDate = [addDays(new Date(), -1), new Date(2001, 0, 6)];
