...
type OpenDialog = (index?: number) => void;
...
function EmblaCarousel({ images, option }: { images: string[]; option: EmblaOptionsType }) {
    // Embla does not read the document's dir; it takes it from the same provider the rest of the UI uses.
    const direction = useDirection();
    const [selectedIndex, setSelectedIndex] = useState(0);
    const [emblaMainRef, emblaMainApi] = useEmblaCarousel({ ...option, direction });
                ...
                <div className="embla-thumbs__viewport mx-auto max-w-28" ref={emblaThumbsRef}>
                    <div className="embla-thumbs__container flex">
                        {images.map((image, index) => (
                            ...
                            >
                                <Img alt="Slide image" src={image} fill sizes="100px" className={"object-cover"} />
                            </div>
                        ))}
                    </div>
                </div>
...
export default function HeaderGallery({ images, gridType = "grid1" }: HeaderGalleryProps) {
    const [isOpen, setIsOpen] = useState(false);
    const [startIndex, setStartIndex] = useState(0);

    const handleOpenDialog: OpenDialog = (index = 0) => {
        setStartIndex(index);
        setIsOpen(true);
    };
...
function HeaderGalleryGrid1({ images, handleOpenDialog }: GridProps) {
    return (
...
function HeaderGalleryGrid2({ images, handleOpenDialog }: GridProps) {
    return (
...
function HeaderGalleryGrid3({ images, handleOpenDialog }: GridProps) {
    return (
...
function HeaderGalleryGrid4({ images, handleOpenDialog }: GridProps) {
    return (
        <header className="relative md:grid md:grid-cols-3 md:gap-x-2">
            <div className="relative aspect-4/5 size-full md:aspect-3/4" onClick={() => handleOpenDialog(0)}>
                {images[0] && (
...
function ButtonShowAllPhotos({ handleOpenDialog }: { handleOpenDialog: OpenDialog }) {
    return (
