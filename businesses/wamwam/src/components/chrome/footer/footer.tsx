...
import SocialsList1 from "@/components/chrome/socials-list1";
import type { CustomLink } from "@/types/domain";
...
function WidgetMenu({ menu }: { menu: WidgetFooterMenu }) {
    return (
        ...
        <div className="relative border-t border-neutral-200 py-24 lg:py-28 dark:border-neutral-700">
            <div className="container grid grid-cols-2 gap-x-5 gap-y-10 sm:gap-x-8 md:grid-cols-4 lg:grid-cols-5 lg:gap-x-10">
                ...
                {widgetMenus.map((menu, index) => (
                    <WidgetMenu key={index} menu={menu} />
                ))}
            </div>
        </div>
    );
