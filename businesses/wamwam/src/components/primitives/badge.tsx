...
// `colors` is kept verbatim from the design system's badge.tsx (its formatting
// included) so the class table diffs cleanly against upstream.
const colors = {
...
export function Badge({ color = "zinc", className, ...props }: BadgeProps & ComponentProps<"span">) {
    return (
        <span
            {...props}
            className={clsx(
...
export function BadgeButton({
    ...
    children,
    ...props
}: BadgeProps & { className?: string; children: ReactNode; ref?: Ref<HTMLElement> } & ClickableProps) {
    const classes = clsx(
        className,
