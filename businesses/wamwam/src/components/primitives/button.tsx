...
import { Clickable, TouchTarget, type ClickableProps } from "@/components/primitives/clickable";

// The `styles` object is kept byte-for-byte from the design system's button.tsx,
// its own formatting included: every class is load-bearing and it is diffed
// against upstream as a block.
const styles = {
  base: [
    // Base
...
/**
 * A button is solid (optionally coloured), outline, or plain — never a mix.
 * `outline` is a boolean rather than the literal `true` because callers toggle
 * it from state; `false` renders exactly like omitting it, since the variant is
 * resolved by truthiness below.
 */
type Variant =
...
type Common = { className?: string; ref?: Ref<HTMLElement> } & ClickableProps;

export type ButtonProps = Variant & { children: ReactNode } & Common;
export type ButtonCircleProps = Variant & { children?: ReactNode } & Common;

/** Solid buttons layer a colour set over `solid`; outline and plain are complete on their own. */
function variantClasses(
    color: keyof typeof styles.colors | undefined,
    outline: boolean | undefined,
    plain: boolean | undefined,
) {
    return outline ? styles.outline : plain ? styles.plain : clsx(styles.solid, styles.colors[color ?? "dark/neutral"]);
}

export function Button({ color, outline, plain, className, children, ...props }: ButtonProps) {
    const classes = clsx(className, styles.base, variantClasses(color, outline, plain));

    return (
        <Clickable {...props} className={classes} buttonClassName="cursor-default">
            {children}
        </Clickable>
    );
}

export function ButtonCircle({ color, outline, plain, className, children = "x", ...props }: ButtonCircleProps) {
    const classes = clsx(className, styles.baseForCircle, variantClasses(color, outline, plain));
