...
import type { ExperienceListing, StayListing } from '@/types/listings'
...
interface Props {
  ...
  listings: StayListing[] | ExperienceListing[]
  cardType: 'stay' | 'experience' | 'real-estate'
}

export default function SectionSliderCards({
  className,
  ...
  const renderCard = (item: StayListing | ExperienceListing) => {
    switch (cardType) {
      case 'stay':
        return <StayCard2 data={item as StayListing} />
      case 'experience':
        return <ExperiencesCard data={item as ExperienceListing} />
      default:
        return <StayCard2 data={item as StayListing} />
    }
  }
