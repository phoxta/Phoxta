...
interface GetawayChip {
  id: string
  icon: IconSvgElement
  title: string
}

const DEMO_DATA: GetawayChip[] = [
  {
    id: '1',
...
// Chips used to point at a placeholder stay-category route; they now pre-fill the
// experience search with the chip's own name.
const chipHref = (item: GetawayChip) => `/experience-search?location=${encodeURIComponent(item.title)}`
      ...
      <div className="relative overflow-hidden mask-[linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] py-5">
        <div className="flex rtl:flex-row-reverse">
          ...
          <div className="animate-marquee flex shrink-0 items-center justify-around gap-6 px-3 sm:gap-16 sm:px-8">
            {DEMO_DATA.map((item) => (
              <Link href={chipHref(item)} key={item.id} className="flex items-center gap-2.5">
