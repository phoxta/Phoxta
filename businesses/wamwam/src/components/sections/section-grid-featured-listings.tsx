import avatar2 from '@/assets/images/avatars/Image-1.png'
import avatar1 from '@/assets/images/avatars/Image-2.png'
...
import ButtonLargeWithIcon from '@/components/primitives/button-large-with-icon'
...
// avatar1 deliberately maps to Image-2 and avatar2 to Image-1: the rendered order is the original's.
const users = [
  {
    id: 1,
    name: 'Alice',
...
function FeaturedListingCard({ stay }: { stay: StayListing }) {
  ...
  return (
    <div className="group/listing relative aspect-6/7 embla__slide basis-full sm:aspect-7/6 lg:aspect-6/7 xl:aspect-7/6">
      ...
      <AppLink className="absolute inset-0" href={`/stay-listings/${listingHandle}`} />
    </div>
  )
}

export default function SectionGridFeaturedListings({
  stayListings = [],
  heading = (
    <>
...
}: SectionGridFeaturedListingsProps) {
