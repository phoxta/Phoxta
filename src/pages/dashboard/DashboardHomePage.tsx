...
/* ---- Glass AI Operator (dashboard home only) -----------------------------
   Every rule below is scoped to .dash-right. OperatorChat and operator-chat.css
   are shared with the console, which keeps its solid panel — the surface people
   work in all day does not change because the home page got a treatment. That
   scoping is also why the avatar arrives as a ::before rather than new markup.

   The mesh sits on .opc itself, so the header, bubbles and composer each blur
   the colour behind them instead of all blurring one flat parent. The first
   layer is a dark veil over the lower third: the mesh ends in pale blue exactly
   where the composer sits, and white placeholder text on pale blue is not
   readable. */
.dash-right .opc {
  border: 1px solid rgba(255, 255, 255, 0.16);
  border-radius: 20px;
  background:
    linear-gradient(180deg, rgba(6, 10, 30, 0) 52%, rgba(6, 10, 30, 0.34) 100%),
    radial-gradient(72% 55% at 10% 6%,  #14173a 0%, rgba(20, 23, 58, 0) 68%),
    radial-gradient(58% 42% at 60% 4%,  #2b5fc4 0%, rgba(43, 95, 196, 0) 66%),
    radial-gradient(52% 40% at 98% 26%, #e2632f 0%, rgba(226, 99, 47, 0) 64%),
    radial-gradient(48% 36% at 28% 40%, #b8386f 0%, rgba(184, 56, 111, 0) 64%),
    radial-gradient(58% 44% at 4%  74%, #2f6fd0 0%, rgba(47, 111, 208, 0) 66%),
    radial-gradient(68% 52% at 74% 96%, #8ccdec 0%, rgba(140, 205, 236, 0) 70%),
    linear-gradient(152deg, #1b1f42 0%, #7a3d86 34%, #2f6fd0 68%, #93cbe8 100%);
}

/* The header becomes an inset glass bar carrying the operator's face. */
.dash-right .opc-head {
...
.dash-right .opc-head::before {
  content: "";
  flex: 0 0 38px; width: 38px; height: 38px; border-radius: 50%;
  background: url(/assets/imgs/pages/home-12/sec-7-avatar-2.webp) center/cover no-repeat;
...
.dash-right .opc-group.mine .opc-bubble {
  background: rgba(12, 14, 28, 0.36);
  border: 1px solid rgba(255, 255, 255, 0.14);
  border-bottom-right-radius: 6px;
}

/* The comp shows a photo beside the operator and nothing beside YOU. */
.dash-right .opc-meta b { color: #fff; }
.dash-right .opc-meta i { color: rgba(255, 255, 255, 0.72); }
.dash-right .opc-group.mine .opc-av { display: none; }
.dash-right .opc-group.theirs .opc-av {
  font-size: 0;
...
.dash-right .opc-day span { color: rgba(255, 255, 255, 0.80); }
.dash-right .opc-empty { color: rgba(255, 255, 255, 0.88); }
.dash-right .opc-starters button {
  background: rgba(255, 255, 255, 0.14);
  border-color: rgba(255, 255, 255, 0.28);
  color: #fff;
}
.dash-right .opc-starters button:hover { border-color: rgba(255, 255, 255, 0.70); color: #fff; }
...
.dash-right .opc-text { color: #fff; }
.dash-right .opc-text::placeholder { color: rgba(255, 255, 255, 0.75); }
.dash-right .opc-clip, .dash-right .opc-send { color: rgba(255, 255, 255, 0.90); }
.dash-right .opc-clip:hover, .dash-right .opc-send:hover { color: #fff; }
.dash-right .opc-mic { background: rgba(14, 15, 26, 0.88); }

/* Without backdrop-filter (Firefox with the flag off, older iOS) the glass is
   just thin white over a bright mesh, which strands the type. Go opaque-dark
