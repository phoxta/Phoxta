...
import { toastError, confirmDanger, reportMutation } from "@/lib/ops/feedback";
...
export default function PromoCodes({ orgId }: { orgId: string }) {
  ...
  return (
    <div className="bg-neutral-0 rounded-4 p-4 border-100">
      ...
      {loadError && <div className="alert alert-warning py-2 px-3 fz-font-sm">{loadError}</div>}
      <form onSubmit={add} className="row g-2 align-items-end mb-3">
        <div className="col-6 col-md-3"><label htmlFor="promo-code" className="fz-font-sm neutral-500 d-block mb-1">Code</label><input id="promo-code" className="form-control form-control-sm rounded-3 text-uppercase" placeholder="WELCOME10" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} /></div>
        ...
        {form.expires && <div className="col-12 fz-font-sm neutral-500">Valid through {new Date(`${form.expires}T12:00:00`).toLocaleDateString()} — customers can use the code until the end of that day.</div>}
      </form>
      {loading ? (
          ...
          {promos.map((p) => (
            <div key={p.id} className="border-100 rounded-3 p-2 px-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
              <span className="fz-font-md">
                ...
                <span className="neutral-500 fz-font-sm"> · {p.kind === "percent" ? `${p.value}% off` : `${formatPrice(p.value, currency)} off`}{p.min_cents > 0 ? ` · min ${formatPrice(p.min_cents, currency)}` : ""}{p.expires_at ? ` · valid through ${new Date(p.expires_at).toLocaleDateString()}` : ""}</span>
              </span>
              <span className="d-flex align-items-center gap-3">
                <button type="button" className={`btn btn-link btn-sm p-0 text-decoration-none fw-600 ${p.active ? "text-success" : "neutral-500"}`} onClick={async () => { if (await reportMutation(updatePromo(p.id, { active: !p.active }), p.active ? "Code paused." : "Code activated.")) reload(); }}>{p.active ? "Active" : "Paused"}</button>
                <button type="button" className="btn btn-link btn-sm p-0 neutral-500 text-decoration-none" onClick={async () => { if (confirmDanger(`Delete ${p.code}? Customers will no longer be able to use it.`)) { if (await reportMutation(deletePromo(p.id), "Code deleted.")) reload(); } }}>Delete</button>
              </span>
            </div>
          ))}
        </div>
