...
import { listPromos, createPromo, updatePromo, deletePromo, type Promo, type PromoKind } from "@/lib/db/ops/promo";
...
export default function PromoCodes({ orgId }: { orgId: string }) {
  ...
  async function add(e: React.FormEvent) {
    ...
    setBusy(false);
    if (error) setErr(error);
    else { setForm({ code: "", kind: "percent", value: "", min: "", expires: "" }); load(); }
  }
          ...
          {promos.map((p) => (
            <div key={p.id} className="border-100 rounded-3 p-2 px-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
              ...
              <span className="d-flex align-items-center gap-3">
                <button type="button" className={`btn btn-link btn-sm p-0 text-decoration-none fw-600 ${p.active ? "text-success" : "neutral-500"}`} onClick={async () => { await updatePromo(p.id, { active: !p.active }); load(); }}>{p.active ? "Active" : "Paused"}</button>
                <button type="button" className="btn btn-link btn-sm p-0 neutral-500 text-decoration-none" onClick={async () => { if (confirm(`Delete ${p.code}?`)) { await deletePromo(p.id); load(); } }}>Delete</button>
              </span>
            </div>
          ))}
        </div>
